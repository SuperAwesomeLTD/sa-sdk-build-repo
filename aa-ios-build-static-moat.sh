...
build="aa-ios-build-static-moat"
# project
...
do
  ...
  find "$source/Pod/Classes/" -iname '*.m' -exec cp \{\} $build/static/src \;
	...
	if [ -d "$source/Pod/Plugin/Moat" ]; then
		find "$source/Pod/Plugin/Moat" -iname '*.h' -exec cp \{\} $build/lib$project/include/$project \;
	fi

	if [ -d "$source/Pod/Libraries" ]; then
	  cp "$source/Pod/Libraries/libSUPMoatMobileAppKit.a" $build/lib$project/
	fi
