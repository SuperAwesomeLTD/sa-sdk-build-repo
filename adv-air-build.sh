...
# set source and repo
source=sa-adobeair-advertiser-sdk
...
do
	...
	if [ $destination = "superawesomeadvertiser" ]; then
		...
		cp superawesomeadvertiser/build/outputs/aar/superawesomeadvertiser-release.aar ../$build/android/superawesomeadvertiser.zip
		cp sadvair/build/outputs/aar/sadvair-release.aar ../$build/android/sadvair.zip
		...
		unzip sadvair.zip -d sadvair && rm sadvair.zip
		cp sadvair/classes.jar sadvair.jar && rm -rf sadvair
...
echo "<id>tv.superawesome.plugins.advertiser.air</id>" >> $extensionFile
...
echo "<nativeLibrary>sadvair.jar</nativeLibrary>" >> $extensionFile
echo "<initializer>tv.superawesome.plugins.advertiser.air.SAdvAIR</initializer>" >> $extensionFile
echo "</applicationDeployment>" >> $extensionFile
...
echo "<initializer>SAdvExtensionInitializer</initializer>" >> $extensionFile
echo "</applicationDeployment>" >> $extensionFile
echo "</platform>" >> $extensionFile
# default platform
