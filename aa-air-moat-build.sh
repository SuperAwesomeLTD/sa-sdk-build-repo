...
do
	...
	# case when it's one of the libraries
		...
		then
			cp $destination/libs/moatlib.jar ../$build/android/moatlib.jar
		fi
...
echo "<packagedDependency>libSUPMoatMobileAppKit.a</packagedDependency>" >> $iosPlatformFile
echo "</packagedDependencies>" >> $iosPlatformFile
